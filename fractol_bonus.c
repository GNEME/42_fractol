...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/04/29 12:31:35 by ggneme            #+#    #+#             */
/*   Updated: 2023/04/29 12:31:55 by ggneme           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_init(int ac, char **argv, t_param *par)
{
	...
	{
		if (ac == 4)
			param.c = complex_init(ft_atof(argv[2], 1), ft_atof(argv[3], 1), 0);
		else
			param.c = complex_init(-0.8, 0.156, 0);
	}
	else if (!ft_cmp(argv[1], "Mandelbrot") && !ft_cmp(argv[1], "Bonus"))
		ft_message(1);
	param.mlx = mlx_init();
